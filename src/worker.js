...
async function analyze(stock,env){
 ...
 return reply({stock,name:official?.name||"",market:official?.market||"尚未辨認",asOf:new Date().toISOString(),finmind:{date:latest.date,close:latest.close},official,verification:check,score:scored,sourceWarnings:[...warnings,...officialResult.errors],links,goodinfo:{mode:"manual_only",note:"可開啟 Goodinfo 手動輸入相同交易日的收盤價核對；未取得自動擷取授權，不宣稱已自動查證。"}});
}
export default {async fetch(request,env,ctx){
 const url=new URL(request.url);
 if(url.pathname==="/api/health")return reply({ok:true,finmindConfigured:!!env.FINMIND_TOKEN,version:"0.2.0",time:new Date().toISOString()});
