...
test("sample scores never become a fake 100 point full-market rank",()=>{const r=selectDailyLeaders(Array.from({length:7},(_,i)=>mk(String(2000+i))),{marketDate:"2026-09-24"});assert.equal(r.stocks.length,5);assert.equal(r.published,false);assert.equal(r.scope,"official_liquidity_prescreen_sample");assert.equal(r.stocks[0].score,null);});
