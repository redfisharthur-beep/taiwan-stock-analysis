# v0.13.0｜新手版首頁、上市櫃公司名冊與全市場官方初篩

本版本直接修改 `main`：放大頂部既有標題圖、簡化首頁股票卡片與說明，將「每日觀察 5 檔」及「價值投資 5 檔」並列，收盤價 **500 元以上（含 500）不納入任何名單**。每檔直接顯示本益比、淨值比、殖利率，及「符合／未達／待查」條件。價值觀察不再把未滿足全部嚴格門檻的候選整批刪光；在同日、有成交且低於 500 元的可用股票中仍依公開估值初篩排序列最多五檔，任何未通過的條件都會標示。

**全市場 vs. 深入分析務必區分：** 每次榜單 API 各讀取 TWSE 上市及 TPEx 上櫃「公司名冊、最新官方日行情、估值表」，以名冊補齊停牌／沒有當日有效成交價的公司，避免只掃成交金額前幾名。所有可辨認的四位數普通股依有效價格、同日成交及官方估值作**全市場初步篩選**；無有效價格、名冊／行情不完整、非四位數與來源缺值各自不冒充為已評完的股票。名冊也可能包含需額外核定證券類別的特殊股票，不能宣稱已涵蓋每一類證券。首頁顯示當次官方資料的實際總數、同日有效價格、合乎價格和成交條件的檔數、排除價格及來源完整狀態，不硬編固定的「全市場檔數」。

**目前不能宣稱每一檔已完整財報、籌碼、技術或新聞評分。** Cloudflare 免費方案單次外部子請求受限：全市場 6 個官方批次資料端點 + 入選的 5 檔各 8 個 FinMind 資料集，最多約 46 次（不含偶發重導向）。首頁批次為節省配額略過逐檔融資融券與獨立新聞核實，對缺值仍標示未評；點入個股的完整查詢才會執行完整資料集及有權限的永豐後端核對。真正「全部股票均完成深度分析」仍需有授權、合法額度且可追蹤的排程、分批持久化與過期更新機制；目前**尚未**部署該整市場深度資料庫，不應以初篩偽稱全面分析。

- `/api/top5`：全市場官方初篩後的每日觀察，含股票收盤、估值檢查、資料覆蓋及深入分析回應狀態。
- `/api/value5`：相同全市場、低於 500 元的估值初篩五檔，無硬門檻剔除，可選擇獨立排除代號。**不等於內在價值、合理價或未來漲幅排名**。
- `/api/health`：版本應為 `0.13.0`。必須等待 Cloudflare 成功重新部署後，以上新介面與 API 才會在正式網址生效。
- Cloudflare 使用 GitHub main 自動或手動部署設定；Render 的 Shioaji 秘鑰保持在 Render，未做全市場券商行情散布。絕對不要將 FinMind、Shioaji Key／Secret 推到公開倉庫。

以上均為篩選線索與學習資料，不構成買賣建議；沒有資料的條件不能視為已達成或失敗。
